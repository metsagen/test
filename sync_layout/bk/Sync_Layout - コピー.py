...
class SyncLayout:
    ...
    def synclayout(self):
        if self.dlg.checkBox.isChecked():
            composerTitle = str(self.dlg.comboBox.currentText())
            #print(composerTitle)
            project = QgsProject.instance()
            projectLayoutManager = project.layoutManager()
            targetLayout = projectLayoutManager.layoutByName(composerTitle)
            
            targetMapList=[]
            for item in targetLayout.items():
                #print(type(item))
                if type(item) is QgsLayoutItemMap:
                    targetMapList.append(item)
            
            targetMap=targetMapList[0]
            #print(targetMap.extent())
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg = SyncLayoutDialog()
        
            self.dlg.checkBox.setCheckState(False)
            self.dlg.checkBox.stateChanged.connect(self.onsynclayout)
            ...
            self.dlg.comboBox.addItems(layoutList)   
